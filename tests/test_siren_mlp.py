...
def test_scaled_sine_layer():
    scale = 1.5
    ip = tf.keras.layers.Input(shape=[2])
    out = tf_siren.ScaledSinusodialRepresentationDense(units=32, scale=scale, w0=1.0)(ip)
...
def test_scaled_sine_layer_with_w0():
    w0 = 2.0
    scale = 1.5

    ip = tf.keras.layers.Input(shape=[2])
    out = tf_siren.ScaledSinusodialRepresentationDense(units=32, scale=scale, w0=w0)(ip)
...
def test_ScaledSIREN_model():
    scale = 1.5
    model = tf_siren.ScaledSIRENModel(32, final_units=32, final_activation='linear',
                                      num_layers=2, w0=1.0, w0_initial=30.0, scale=scale)
    model.compile('adam', 'mse')
...
def test_ScaledSIREN_model_with_scale():
    w0 = 2.0
    scale = 1.5

    model = tf_siren.ScaledSIRENModel(32, final_units=32, final_activation='linear',
                                      num_layers=2, w0=w0, w0_initial=30.0, scale=scale)
    model.compile('adam', 'mse')
